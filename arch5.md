...
### 1.4 iSCSI 구성
```
/> /backstores/block create name=db_disk dev=/dev/sdb
/> /iscsi create wwn=iqn.2025-07.com.example:storage1
/> /iscsi/iqn.2025-07.com.example:storage1/tpg1/luns create /backstores/block/db_disk
/> /iscsi/iqn.2025-07.com.example:storage1/tpg1/acls create iqn.2025-07.com.example:db
/> saveconfig
/> exit
```
각 명령에 대한 설명은... 나중에...
...
InitiatorName=iqn.2025-07.com.example:db
```
...
### 2.4 타겟 검색 및 로그인
```
sudo iscsiadm -m discovery -t st -p iSCSI 서버 ip 주소
sudo iscsiadm -m node -T iqn.2025-07.com.example:storage1 -p iSCSI 서버 ip 주소 -l
```
...
ls -Zd /mnt/mysql # 적용여부 확인 아래처럼 나오면 된다
drwxr-xr-x. root root system_u:object_r:mysqld_db_t:s0 /mnt/mysql
```
